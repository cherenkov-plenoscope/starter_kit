...
for site_key in irf_config["config"]["sites"]:
    for particle_key in ["gamma"]:
        ...
        idxs_no_leakage = event_table["features"][spt.IDX][mask_no_leakage]

        cut_idxs = spt.intersection(
            [idxs_primary, idxs_features, idxs_in_energy_bin, idxs_no_leakage]
        )
        ...
        _psf_events = spt.sort_table_on_common_indices(
            table=events, common_indices=cut_idxs,
        )
        ...
        (psf_train, psf_test) = sklearn.model_selection.train_test_split(
            psf_events, test_size=psf_test_sample_fraction, random_state=13
        )

        x_psf_train = make_pfs_features(psf_train)
        y_psf_train = np.array(
            mdfl.discovery._az_zd_to_cx_cy(
                azimuth_deg=np.rad2deg(psf_train["primary.azimuth_rad"]),
                zenith_deg=np.rad2deg(psf_train["primary.zenith_rad"]),
            )
        ).T
        ...
        y_psf_test = np.array(
            mdfl.discovery._az_zd_to_cx_cy(
                ...
                zenith_deg=np.rad2deg(psf_test["primary.zenith_rad"].values),
            )
        ).T
        ...
        fig.savefig(
            os.path.join(
                pa["out_dir"], "{:s}_cx_true_vs_reco.jpg".format(site_key)
            )
        )
        ...
        fig.savefig(
            os.path.join(
                pa["out_dir"], "{:s}_cy_true_vs_reco.jpg".format(site_key)
            )
        )
