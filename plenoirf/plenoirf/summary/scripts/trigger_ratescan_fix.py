...
def make_trigger_indices(trigger_table, threshold):
    mask = make_trigger_mask(trigger_table, threshold)
...
trigger_thresholds_pe = np.array(sum_config['trigger_thresholds_pe'])
trigger_threshold = trigger_thresholds_pe[3 + sum_config['nominal_trigger_threshold_idx']]
trigger_config = {
    "chile": {"threshold": trigger_threshold},
    "namibia": {"threshold": trigger_threshold},
}
...
for site_key in irf_config['config']['sites']:
    ...
    for particle_key in irf_config['config']['particles']:
        ...
        cosmic_table[particle_key] = event_table
    ...
    for particle_key in irf_config['config']['particles']:
        ...
        nsb_indices = cosmic_table[particle_key]['trigger'][spt.IDX][mask_nsb]

        nsb_table[particle_key] = spt.cut_table_on_indices(
            table=cosmic_table[particle_key],
            structure=irf.table.STRUCTURE,
            common_indices=spt.dict_to_recarray({spt.IDX: nsb_indices})
        )
    ...
    for particle_key in irf_config['config']['particles']:
        nsb_exposure_time += (
            nsb_table[particle_key]['trigger'].shape[0]*
            EXPOSURE_TIME_PER_EVENT
        )
        for t in range(NUM_THRESHOLDS):
            nsb_trigger_indices = irf.analysis.light_field_trigger_modi.make_indices(
                trigger_table=nsb_table[particle_key]['trigger'],
                threshold=trigger_thresholds_pe[t],
                modus=trigger_modus
            )
            num_nsb_triggers[t] += len(nsb_trigger_indices)
    ...
    _off_indices = cosmic_table['gamma']['primary'][spt.IDX][_off_mask]

    gamma_table = mrg_pri_grd = spt.cut_table_on_indices(
        table=cosmic_table['gamma'],
        structure=irf.table.STRUCTURE,
        common_indices=spt.dict_to_recarray({spt.IDX: _off_indices}),
    )

    for t in range(NUM_THRESHOLDS):

        gamma_f_detected_indices = irf.analysis.light_field_trigger_modi.make_indices(
            trigger_table=gamma_table['trigger'],
            threshold=trigger_thresholds_pe[t],
            modus=trigger_modus,
        )

        gamma_f_detected = spt.make_mask_of_right_in_left(
            left_indices=gamma_table['primary'][spt.IDX],
            right_indices=gamma_f_detected_indices
        )
    ...
    for particle_key in airshower_rates:
        ...
        for t in range(NUM_THRESHOLDS):
            ...
            f_detected_indices = irf.analysis.light_field_trigger_modi.make_indices(
                trigger_table=cosmic_table[particle_key]['trigger'],
                threshold=trigger_thresholds_pe[t],
                modus=trigger_modus
            )

            f_detected = spt.make_mask_of_right_in_left(
                left_indices=cosmic_table[particle_key]['primary'][spt.IDX],
                right_indices=f_detected_indices
            )
