...
num_c_bins = 50
c_bin_edges_deg = np.linspace(-fov_radius_deg, fov_radius_deg, num_c_bins + 1)
...
for site_key in irf_config["config"]["sites"]:
    ...
    for energy_bin in range(num_energy_bins):
        ...
        delta_hist = np.array(
            psf[site_key][particle_key]["theta_square_histogram_vs_energy"][
                "mean"
            ][energy_bin]
        )
        ...
        delta_hist_unc = np.array(
            psf[site_key][particle_key]["theta_square_histogram_vs_energy"][
                "relative_uncertainty"
            ][energy_bin]
        )

        containment_angle_deg = psf[site_key][particle_key][
            "containment_angle_vs_energy"
        ]["mean"][energy_bin]

        containment_angle_rel_unc_deg = psf[site_key][particle_key][
            "containment_angle_vs_energy"
        ]["relative_uncertainty"][energy_bin]
        ...
        ax.axvline(
            x=containment_angle_deg ** 2, color="k", linestyle="-", alpha=0.75
        )
        ...
        # ----------------------
        dev_deg = np.array(
            psf[site_key][particle_key]["point_spread_distribution_vs_energy"][
                "deviation"
            ][energy_bin]
        )
        ...
        if len(dev_deg) > 0:
            img = np.histogram2d(
                dev_deg[:, 0], dev_deg[:, 1], c_bin_edges_deg
            )[0]
        else:
        ...
        fig.savefig(
            os.path.join(
                pa["out_dir"],
                "{:s}_gamma_{:06d}_psf.jpg".format(site_key, energy_bin),
            )
        )
        plt.close(fig)
    ...
    fix_onregion_radius = np.array(
        psf[site_key][particle_key]["containment_angle_for_fix_onregion"][
            "containment_angle"
        ]
    )
    ...
    ax.spines["top"].set_color("none")
    ax.spines["right"].set_color("none")
    ax.axhline(y=fix_onregion_radius, color="k", linestyle="--", alpha=0.75)
    ax.grid(color="k", linestyle="-", linewidth=0.66, alpha=0.1)
    fig.savefig(
        os.path.join(
