...
pixel_spacing_rad = light_field_geometry.sensor_plane2imaging_system.pixel_FoV_hex_flat2flat
...
for obj, object_distance in enumerate(object_distances):
    ...
    cpath = os.path.join(pa["out_dir"], 'lixel_to_pixel_{:06d}.json'.format(obj))
    ...
    for i, pixel_id in enumerate(central_seven_pixel_ids):
        ...
        valid_polygons = []
        additional_colored_lixels = np.zeros(light_field_geometry.number_lixel, dtype=np.bool)
    ...
    irf.summary.figure.add_rays_to_ax(ax=ax2, object_distance=t, linewidth=.5)
    ax2.text(
        x=0.1,
        y=2*t,
