...
cradial2_bin_edges_deg2 = np.linspace(
    0, max_cradial2_deg2, num_cradial_bins + 1
)
...
for site_key in irf_config["config"]["sites"]:
    for particle_key in irf_config["config"]["particles"]:
        ...
        counts_thrown = np.histogram(
            delta_deg ** 2, bins=cradial2_bin_edges_deg2
        )[0]
        ...
        offaxis = np.hypot(
            reconstructed_cx_cy[:, 0], reconstructed_cx_cy[:, 1]
        )
        offaxis_deg = np.rad2deg(offaxis)
        offaxis2_deg2 = offaxis_deg ** 2.0

        counts_reco = np.histogram(
            offaxis2_deg2, bins=cradial2_bin_edges_deg2
        )[0]
        ...
        ax.text(x=0.8, y=0.8, s=": thrown", color="k", transform=ax.transAxes)
        ax.text(
            x=0.8,
            y=0.85,
            s="- reconstructed",
        ...
        fig.savefig(
            os.path.join(pa["out_dir"], site_key + "_" + particle_key + ".jpg")
        )
        plt.close(fig)
