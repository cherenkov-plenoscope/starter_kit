...
def make_gh_features(gh_df):
    ...
    norm_paxel_intensity_offset = np.sqrt(_slope) / np.sqrt(71.0 / 2.0)
    ...
    return np.array(
        [
            ...
            np.log10(gh_df["features.image_smallest_ellipse_object_distance"])
            - 3,
...
for site_key in irf_config["config"]["sites"]:
    ...
    gamma_table = spt.sort_table_on_common_indices(
        table=gamma_table, common_indices=cut_idxs
    )
    ...
    _full_proton_table = spt.read(
        path=os.path.join(
            pa["run_dir"], "event_table", site_key, "proton", "event_table.tar"
        ),
        structure=irf.table.STRUCTURE,
    )
    _mask_no_leakage = (
        _full_proton_table["features"][
            "image_infinity_num_photons_on_edge_field_of_view"
        ]
        < MAX_LEAKAGE_PE
    )
    ...
    proton_table = spt.sort_table_on_common_indices(
        table=proton_table, common_indices=idxs_no_leakage
    )
    ...
    _full_helium_table = spt.read(
        path=os.path.join(
            pa["run_dir"], "event_table", site_key, "helium", "event_table.tar"
        ),
        structure=irf.table.STRUCTURE,
    )
    _mask_no_leakage = (
        _full_helium_table["features"][
            "image_infinity_num_photons_on_edge_field_of_view"
        ]
        < MAX_LEAKAGE_PE
    )
    ...
    helium_table = spt.sort_table_on_common_indices(
        table=helium_table, common_indices=idxs_no_leakage
    )
    ...
    (gh_train, gh_test) = sklearn.model_selection.train_test_split(
        full_df, test_size=0.5, random_state=13
    )
    ...
    fpr_gh, tpr_gh, thresholds_gh = sklearn.metrics.roc_curve(
        y_true=y_gh_test,
        y_score=y_gh_scaler.inverse_transform(gh_mlp.predict(x_gh_test_s)),
    )
    ...
    roc_gh_path = os.path.join(
        pa["out_dir"], "{:s}_roc_gamma_hadron".format(site_key)
    )
    ...
    ax = fig.add_axes([0.2, 0.2, 0.72, 0.72])
