...
def estimate_num_grid_bins_with_scatter_limit(
    ...
    grid_threshold_num_photons = irf_config['config']['grid']['threshold_num_photons']
    ...
    scatter_radius_bins = int(
        np.round(_scatter_radius_m/grid_bin_xy_width)
    )

    # range of bins to search for
    x_bin_range = grid_num_bins_radius + x_center_bin + np.arange(
        -scatter_radius_bins,
        scatter_radius_bins+1
    )
    y_bin_range = grid_num_bins_radius + y_center_bin + np.arange(
        -scatter_radius_bins,
        scatter_radius_bins+1
    )
...
for site_key in irf_config['config']['sites']:
    for particle_key in irf_config['config']['particles']:
        ...
        for ii in range(airshower_table['grid']['idx'].shape[0]):
            d_grid[
                airshower_table['grid']['idx'][ii]
            ] = airshower_table['grid'][ii]
        d_energy = {}
        for ii in range(airshower_table['primary']['idx'].shape[0]):
            d_energy[
                airshower_table['primary']['idx'][ii]
            ] = airshower_table['primary']['energy_GeV'][ii]
        ...
        grid_out['area_thrown_m2'] = rr['scatter_num_bins_thrown']*grid_bin_xy_width**2
        grid_out['num_bins_above_threshold'] = rr['scatter_num_bins_above_threshold']
