...
def cut_quality(
    ...
    return spt.intersection([
        idxs_sufficient_size,
        idxs_acceptable_leakage
    ])


def cut_energy_bin(
    primary_table,
    lower_energy_edge_GeV,
    upper_energy_edge_GeV,
):
    mask_energy_bin = (
...
def cut_reconstructed_source_in_true_onregion(
    ...
    for evt in range(table['features'].shape[0]):
        ...
        (rec_cx, rec_cy) = gamma_direction.estimate(
            ...
            image_infinity_cx_mean=ta['features']['image_infinity_cx_mean'][evt],
            image_infinity_cy_mean=ta['features']['image_infinity_cy_mean'][evt],
        )
...
def cut_reconstructed_source_in_random_possible_onregion(
    feature_table,
    ...
    for evt in range(feature_table.shape[0]):
        true_cx, true_cy = cpw.random_distributions.draw_x_y_in_disc(
            radius=np.deg2rad(radial_angle_to_put_possible_onregion_deg)
        )
