...
def test_seed_limit_run_id():
    seed = plenoirf.random_seed.random_seed_based_on(
        run_id=plenoirf.random_seed.NUM_RUN_IDS - 1, airshower_id=1
    )
    assert (
        plenoirf.random_seed.NUM_RUN_IDS - 1
    ...
    with pytest.raises(AssertionError) as e:
        plenoirf.random_seed.random_seed_based_on(
            run_id=plenoirf.random_seed.NUM_RUN_IDS, airshower_id=1
        )
...
def test_seed_combinations():
    ...
    run_ids = np.random.uniform(
        0, plenoirf.random_seed.NUM_RUN_IDS - 1, size=300
    ).astype("i4")
    airshower_ids = np.random.uniform(
        0, plenoirf.random_seed.NUM_AIRSHOWER_IDS_IN_RUN - 1, size=300
    ).astype("i4")
...
def test_template_string():
    np.random.seed(0)
    run_ids = np.random.uniform(
        0, plenoirf.random_seed.NUM_RUN_IDS - 1, size=30
    ).astype("i4")
    airshower_ids = np.random.uniform(
        0, plenoirf.random_seed.NUM_AIRSHOWER_IDS_IN_RUN - 1, size=30
    ).astype("i4")
