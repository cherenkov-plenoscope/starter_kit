...
def make_cherenkov_bunches(
    ...
    cherenkov_bunches[:, cpw.IX] = np.random.normal(
        loc=x_m*cpw.M2CM,
        scale=x_std_m*cpw.M2CM,
        size=num_bunches)
    cherenkov_bunches[:, cpw.IY] = np.random.normal(
        loc=y_m*cpw.M2CM,
        scale=y_std_m*cpw.M2CM,
        size=num_bunches)
...
def test_grid_assign_head_on():
    ...
    cherenkov_bunches = make_cherenkov_bunches(
        cx_deg=0.0, cx_std_deg=1.0,
        cy_deg=0.0, cy_std_deg=1.0,
        x_m=0.0, x_std_m=100.0,
        y_m=0.0, y_std_m=100.0,
        num_bunches=10*1000
    )
...
def test_shower_cx_moves_out_of_fov():
    ...
    for shower_cx_deg in expectation:
        cherenkov_bunches = make_cherenkov_bunches(
            cx_deg=shower_cx_deg, cx_std_deg=1.0,
            cy_deg=0.0, cy_std_deg=1.0,
            x_m=0.0, x_std_m=100.0,
            y_m=0.0, y_std_m=100.0,
            num_bunches=10*1000
...
def test_shower_x_moves_not_counteracted():
    ...
    for s in scenarios:
        cherenkov_bunches = make_cherenkov_bunches(
            cx_deg=0.0, cx_std_deg=1.0,
            cy_deg=0.0, cy_std_deg=1.0,
            x_m=scenarios[s]['core_wrt_obslvl'], x_std_m=10.0,
            y_m=0.0, y_std_m=10.0,
            num_bunches=10*1000
        )
        result = plenoirf.grid.assign(
            cherenkov_bunches=cherenkov_bunches,
        ...
        assert np.abs(
            core_x_wrt_plenoscope -
            scenarios[s]['core_wrt_obslvl']
        ) < 1e2
