...
cfg = {
    ...
    "grid": {
        ...
        "num_bins_above_threshold": 10*np.array([
                        1, 2, 4, 8, 4,  2,  1,  4,   16,  64,  256]),
...
def create_dummy_table(
    ...
    cherenkovsize['num_bunches'] = cherenkovsize['num_bunches'].astype(np.uint64)
    cherenkovsize['num_photons'] = 0.97*cherenkovsize['num_bunches']
    t['cherenkovsize'] = cherenkovsize

    _plvl = cut_level(
        level=lvl,
        mask=cherenkovsize['num_bunches'] > 0)
    ...
    cherenkovpool['cx_median_rad'] = np.random.normal(
        loc=0.0,
        scale=config['max_scatter_rad'],
        size=_plvl['num'])
    cherenkovpool['cy_median_rad'] = np.random.normal(
        loc=0.0,
        scale=config['max_scatter_rad'],
        size=_plvl['num'])
    cherenkovpool['x_median_m'] = (
        _plvl['magnet_cherenkov_pool_x_m'] +
        (1e3/_plvl['energy_GeV'])*np.random.normal(
            loc=0.0,
            scale=1.0,
            size=_plvl['num'])
    )
    cherenkovpool['y_median_m'] = (
        _plvl['magnet_cherenkov_pool_y_m'] +
        (1e3/_plvl['energy_GeV'])*np.random.normal(
            loc=0.0,
            scale=1.0,
            size=_plvl['num'])
    ...
    grid['field_of_view_radius_deg'] = config[
        'grid']['field_of_view_radius_deg']*lvl['ones']
    ...
    grid['num_bins_above_threshold'] = grid['num_bins_above_threshold'].astype(np.uint64)
    ...
    _glvl = cut_level(
        level=lvl,
        mask=grid['num_bins_above_threshold'] > 0)
    ...
    cherenkovsizepart['num_bunches'] = cherenkovsizepart['num_bunches'].astype(np.uint64)
    ...
    _pplvl = cut_level(
        level=_glvl,
        mask=cherenkovsizepart['num_bunches'] > 0)
    ...
    cercls['num_true_positives'] = 0.76*trigger['num_cherenkov_pe'][past_trigger_mask]
    cercls['num_false_negatives'] = 0.2*trigger['num_cherenkov_pe'][past_trigger_mask]
    cercls['num_false_positives'] = 0.2*trigger['num_cherenkov_pe'][past_trigger_mask]
    ...
    features['num_photons'] = 1.2*trigger['num_cherenkov_pe'][past_trigger_mask]
    ...
    features['light_front_cx'] = np.random.normal(
        loc=0.0,
        scale=np.deg2rad(3.25),
        size=_ptlvl['num'])
    features['light_front_cy'] = np.random.normal(
        loc=0.0,
        scale=np.deg2rad(3.25),
        size=_ptlvl['num'])
    features['image_infinity_cx_mean'] = (
        features['light_front_cy'] +
        np.random.normal(
            loc=0.0,
            scale=np.deg2rad(0.25),
            size=_ptlvl['num']
        )
    )
    features['image_infinity_cy_mean'] = (
    ...
    features['image_infinity_cy_std'] = np.random.normal(
        ...
        scale=np.deg2rad(0.1),
        size=_ptlvl['num'])
    features['image_infinity_num_photons_on_edge_field_of_view'] = _ptlvl['ones']
    ...
    features['image_half_depth_shift_cy'] = np.random.normal(
        ...
        scale=np.deg2rad(0.5),
        size=_ptlvl['num'])
    features['image_smallest_ellipse_num_photons_on_edge_field_of_view'] = _ptlvl['ones']
