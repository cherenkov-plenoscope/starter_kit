...
def append_random_edge(
    mesh, start_vkey, radius_xy, min_depth, max_particles, N, prng
):
    start_pos = mesh["vertices"][start_vkey]
    print(start_vkey, start_pos)
    if start_pos[2] > min_depth:

        num_new = num_particles_next_gen(
            max_particles=max_particles, prng=prng
        )
        for n in range(num_new):
            ...
            end_pos = np.array(
                [
                    start_pos[0]
                    + prng.uniform(-radius_xy, radius_xy) * 0.001 * dz,
                    start_pos[1]
                    + prng.uniform(-radius_xy, radius_xy) * 0.001 * dz,
                    start_pos[2] - dz,
                ]
