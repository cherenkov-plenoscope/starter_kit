...
for run_path in runpaths:
    ...
    lixels2pixel = image_rays.pixel_ids_of_lixels_in_object_distance(object_distance)

    image_sequence = np.zeros(
        shape=(event.light_field.number_time_slices, event.light_field.number_pixel), 
        dtype=np.uint16)

    for lix, lixel2pixel in enumerate(lixels2pixel):
        if valid[lix]:                                 
            image_sequence[:,lixel2pixel] += event.light_field.sequence[:,lix]
...
diff = pl.Image(
    refocussed_images[1].intensity.astype('float') - refocussed_images[0].intensity.astype('float'),
    target_cx, 
    target_cy)
