    ...
    with tempfile.TemporaryDirectory (prefix='test_corsika_') as tmp:
        ...
        return {
            "mean_prob_to_reach_obslevel":
                float(np.mean(cpb.probability_to_reach_observation_level)),
            "num_bunches":
                int(cpb.probability_to_reach_observation_level.shape[0]),
            "median_wavelength":
                float(np.median(cpb.wavelength))}


def test_corsika_atmospheric_absorbtion():
    observation_levels = np.linspace(1.e3, 6.e3, 4)
    i = {}
    i["ATMABS"] = 0
    i["OBSLEV"] = 1
    i["MNPROB"] = 2
    ...
    for atmabs in [True, False]:
        for observation_level in observation_levels:
            ...
            results[idx, i["ATMABS"]] = 1. if atmabs else 0.
    ...
    atmabs_T = results[:, i["ATMABS"]] == 1.
    atmabs_F = results[:, i["ATMABS"]] == 0.
    ...
    np.testing.assert_approx_equal(
        actual=fit_atmabs_T[0],
        desired=3.8,
        significant=1)

    np.testing.assert_approx_equal(
        actual=fit_atmabs_F[0],
        desired=0.,
        significant=2)
    ...
    np.testing.assert_approx_equal(
        actual=fit_wvl_atmabs_T[0],
        desired=-0.00586,
        significant=3)

    np.testing.assert_approx_equal(
        actual=fit_wvl_atmabs_F[0],
        desired=-0.000017,
        significant=1)
