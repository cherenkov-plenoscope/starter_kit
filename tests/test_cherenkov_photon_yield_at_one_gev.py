...
with tempfile.TemporaryDirectory(prefix="test_corsika_") as tmp:
    ...
    cpw.corsika_vanilla(
        steering_card=steering_card(
            atmospheric_absorbtion=False, num_shower=num_shower
        ),
    ...
    for event in run:
        ...
        cor_first_interaction_altitude.append(
            -1.0 * evth[cpw.I.EVTH.Z_FIRST_INTERACTION_CM] * 1e-2
        )

        assert np.sum(cpb.probability_to_reach_observation_level < 0.0) == 0
        assert np.sum(cpb.probability_to_reach_observation_level > 1.0) == 0
        ...
        cor_hist_emission_altitude.append(
            np.histogram(
                cpb.emission_height,
                weights=cpb.probability_to_reach_observation_level,
                bins=bin_edges_emission_altitude,
            )[0]
        )
...
for bremsstrahlungs_multiplicity in [1, 6]:
    ...
    cor_hist_emission_altitude_normalized = np.sum(
        cor_hist_emission_altitude, axis=0
    ) / np.sum(cor_num_cherenkov_photons)

    tas_hist_emission_altitude_normalized = np.sum(
        tas_hist_emission_altitude, axis=0
    ) / np.sum(tas_num_cherenkov_photons)
    ...
    cor_hist_wavelength_normalized = np.sum(
        cor_hist_wavelength, axis=0
    ) / np.sum(cor_num_cherenkov_photons)
    tas_hist_wavelength_normalized = np.sum(
        tas_hist_wavelength, axis=0
    ) / np.sum(tas_num_cherenkov_photons)
    ax.plot(
    ...
    bin_edges_num_cherenkov_photons = np.linspace(0.0, 3e5, 25)
    cor_hist_num_cherenkov_photons = np.histogram(
        cor_num_cherenkov_photons, bins=bin_edges_num_cherenkov_photons
    )[0]
    tas_hist_num_cherenkov_photons = np.histogram(
        tas_num_cherenkov_photons, bins=bin_edges_num_cherenkov_photons
    )[0]
    ...
    bin_edges_first_interaction_altitude = np.linspace(
        1e3, 100e3, int(2 * np.sqrt(num_shower))
    )
    cor_hist_first_interaction_altitude = np.histogram(
        cor_first_interaction_altitude,
        bins=bin_edges_first_interaction_altitude,
    )[0]
